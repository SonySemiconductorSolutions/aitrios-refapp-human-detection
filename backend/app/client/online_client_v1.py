...
class OnlineConsoleClientV1(ClientInferface[ConfigurationV1]):
    ...
    def get_images_and_inferences(
        ...
        try:
            ...
            response = insight_api.get_images(
                ...
                order_by="ASC",  # To align with API v2 specification
                _request_timeout=self.timeout,
            )

            for data in response.images:
                numeric_timestamp = data.name.split(".")[0]
                base64_image = data.contents
            ...
            if collection:
                # "yyyyMMddHHmmssfff"
                from_datetime_numeric = collection[0]["timestamp"]
                to_datetime_numeric = collection[-1]["timestamp"]
                filter_by_timestamp = f'EXISTS(SELECT VALUE i FROM i IN c.Inferences WHERE i.T >= "{from_datetime_numeric}" AND i.T <= "{to_datetime_numeric}")'

                response = insight_api.get_inference_results(
                    device_id=device_id,
                    number_of_inferenceresults=limit,
                    raw=1,  # raw InferenceResults
                    filter=filter_by_timestamp,
                    _request_timeout=self.timeout,
                )
                ...
                inference_dict = {}
                if response:
                    for inference_data in response:
                        inference = inference_data.inference_result.inferences[0].o
                        inference_timestamp = (
                            inference_data.inference_result.inferences[0].t
                        )
                ...
                for data in collection:
                    ...
                    if image_timestamp in inference_dict:
                        data["inference"] = inference_dict[image_timestamp]["inference"]

            logger.info(
                f"Successfully retrieved inference data for device ID '{device_id}'"
            )
            return collection
        except ApiException as api_error:
    ...
    def get_inferences(
        ...
        logger.debug(
            f"Fetching inference results for device ID '{device_id}' from {from_datetime} to {to_datetime}"
        )
        ...
        try:
            ...
            # dateime-> str "yyyyMMddHHmmssfff"
            from_datetime_numeric = from_datetime.strftime("%Y%m%d%H%M%S%f")[:-3]
            to_datetime_numeric = to_datetime.strftime("%Y%m%d%H%M%S%f")[:-3]
            filter_by_timestamp = f'EXISTS(SELECT VALUE i FROM i IN c.Inferences WHERE i.T >= "{from_datetime_numeric}" AND i.T <= "{to_datetime_numeric}")'
            ...
            if response[0]:
                for inference_data in response:
                    inference_data = {
                        "timestamp": inference_data.inference_result.inferences[0].t,
                        "inference": inference_data.inference_result.inferences[0].o,
                    }
